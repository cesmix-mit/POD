...
//         // estimate memory size to be allocated
//         this->estimate_memory(data);    
//         // allocate memory
...
{        
    ...
    if (numblocks > 1000) pod_error("number of computation blocks can not be more than 1000. This error can be fixed by increasing the number of atoms per computation block.");
}
...
{           
    ...
    for (int i = 0; i< numblocks; i++) { // loop over each computation block
        ...
        //podptr->print_matrix("localdesc", nat, nd1234, tmpmem, nat);
...
{                   
    ...
    // compute energy and effective coefficients
    energy = podptr->calculate_energy(forcecoeff, gd, podcoeff);    
...
{                       
    ...
    for (int i = 0; i< numblocks; i++) { // loop over each computation block
        ...
        for (int j = 0; j<nat; j++)
            eatom[ilist[atomblocks[i] + j]] = ea[i];
    }
...
{        
    podptr->podArraySetValue(force, 0.0, dim*ntotalatom);    
    for (int i = 0; i< numblocks; i++) { // loop over each computation block
        ...
        // compute atomic force for this computation block
        podptr->calculate_force(force, forcecoeff, rij, tmpmem, numneighsum, 
                typeai, idxi, ai, aj, ti, tj, nat, nij);        
    }        
...
{   
    double eatom[data.num_atom_max];
    double f[dim*data.num_atom_max];
    double f2[dim*data.num_atom_max];
...
//             double e1 = this->podeatom(eatom, x, a, b, c, atomtypes, natom);
//             double e2 = podptr->podArraySum(eatom, natom);
//                     
// //             podptr->print_matrix("energycoeff", 1, podptr->pod.nd1234, energycoeff, 1);
// //             podptr->print_matrix("eatom", 1, natom, eatom, 1);
//             
//             this->podforce(f2, x, a, b, c, atomtypes, natom);
//             printf("%d %g %g %g %g\n", ci, energy, e1, e2, podptr->podArrayErrorNorm(f, f2, nforce));
